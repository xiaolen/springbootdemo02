//Windows IE32
var PGEdit_IE32_CLASSID="84333F1F-C6E0-4772-B147-E29473C8EB03";
var PGEdit_IE32_CAB="";
var PGEdit_IE32_EXE="spdbsign_v7.0.zip";
var PGEdit_IE32_VERSION="1.0.0.1";
//Windows IE64
var PGEdit_IE64_CLASSID="84333F1F-C6E0-4772-B147-E29473C8EB03";
var PGEdit_IE64_CAB="";
var PGEdit_IE64_EXE="spdbsign_v7.0.zip";
var PGEditt_IE64_VERSION="1.0.0.1";
//Windows 非IE
var PGEdit_FF="spdbsign_v7.0.zip";
var PGEdit_FF_VERSION="1.0.0.4";
//Win10Edge / Chrome42+
var PGEdit_Edge="spdbsign_v7.0.zip";
var PGEdit_Edge_Mac="SPDBEdge.pkg";
var PGEdit_Edge_VERSION="1.0.0.1";
//Mac OS
var PGEdit_MacOs="SPDB.pkg";
...
var isInstalled = -1,objVersion = "",interv,urls,logFlag = false,onceInterv={},iterArray=[];
var inFlag = {},datac,RZCIJSON,mingflag = false;//控制是否能输入
	...
	jQuery.extend(jQuery.pge, {
		...
		prototype: {
			...
			pwdVerify : function() {
				...
				if (!isInstalled) {
					...
					try {
						var control = this.pwdGetEById(this.settings.pgeId);
						if (this.osBrowser==1 || this.osBrowser==3) {
							control.input11 = this.settings.setDictionaryFilter;
							 code = control.output47;
						} else if (this.osBrowser==2) {
							control.input(902, this.settings.setDictionaryFilter);
							code = control.output(902);
						}else if (this.osBrowser==6 || this.osBrowser==8) {
							control.input19=this.settings.setDictionaryFilter;
			...
			verify:function(){
				...
				if(this.pwdLength() == 0){
					throw 'not installed';
				}else if(this.pwdLength() < this.settings.pgeMinlength|| this.pwdLength() > this.settings.pgeMaxlength){
					result = -2;
				}else if(this.pwdValid() == 1){
					result = -3;
				}else if(this.settings.setDictionaryFilter != ""&&this.pwdVerify()==1){
					result = -5;
				}
				return result;
			},
			...
			getPinValue: function() {
				...
				if (!isInstalled) {
					...
					try {
						var control = this.pwdGetEById(this.settings.pgeId);
						if (this.osBrowser==1 || this.osBrowser==3) {
							control.input7 = this.settings.pgeCert1;
							control.input8 = this.settings.pgeCert2;
							control.input9 = this.settings.setTimes;
							//control.input11 = this.settings.setDictionaryFilter;
							code = control.output28;
						} else if (this.osBrowser==2) {
							control.input(900, this.settings.pgeCert1);
							control.input(901, this.settings.pgeCert2);
							control.input(903, this.settings.setTimes);
							code = control.output(900);
						}else if (this.osBrowser==6 || this.osBrowser==8) {
							control.input8 = this.settings.pgeCert1;
							control.input18 = this.settings.pgeCert2;
							code = control.get_output23(this.settings.setTimes);
						}else if (this.osBrowser == 10 || this.osBrowser == 11){
							var id = this.settings.pgeWindowID;
							INCJSON = {"interfacetype" : 1,"data" : {}};
							INCJSON.id = id ;
							INCJSON.data.oem_903=this.settings.setTimes;
							datac = this.getEnStr(this.settings.pgeRZRandNum,INCJSON);
							RZCIJSON = {"rankey":this.settings.pgeRZRandNum,"datab":this.settings.pgeRZDataB,"datac":datac};
							this.pwdGetData(RZCIJSON);
				...
				if (!isInstalled) {
					...
					try {
						...
						if (this.osBrowser==1 || this.osBrowser==3) {
							...
							control.input18 = this.settings.pgeCert2;
							code = control.get_output26(this.settings.setTimes);
						}else  if (this.osBrowser == 10 || this.osBrowser == 11){
				...
				if (!isInstalled) {
					...
					try {
						...
						if (this.osBrowser==1 || this.osBrowser==3) {
							...
							code = control.get_output24(this.settings.setTimes);
						}else if (this.osBrowser == 10 || this.osBrowser == 11){
							var id = this.settings.pgeWindowID;
			...
			getVersion : function() {
				try {
					if (this.osBrowser==1 || this.osBrowser==3) {
						var comActiveX = new ActiveXObject("SPDBPassGuardX.PassGuard.1"); 
						return comActiveX.output35;
					}else if(this.osBrowser == 2 || this.osBrowser == 6 || this.osBrowser == 8 ){
						var arr = new Array(),pge_version;
				    	var pge_info = navigator.plugins['SPDBPassGuardX'].description;
			...
			checkOsBrowser: function() {
				...
				if((navigator.platform =="Win32") || (navigator.platform =="Windows")){
					...
					}else if(navigator.userAgent.indexOf("Edge")>0 || navigator.userAgent.indexOf("Chrome")>0 || navigator.userAgent.indexOf("Firefox")>0){
							userosbrowser = 10;
					...
					}else if(navigator.userAgent.indexOf("Edge")>0 || navigator.userAgent.indexOf("Firefox")>0 || navigator.userAgent.indexOf("Chrome")>0){
						userosbrowser = 10;
						this.pgeditFFExe = PGEdit_Edge;
					}else{
						userosbrowser=2;
			...
			getpgeHtml: function() {
				if (this.osBrowser==1 || this.osBrowser==3) {
					return '<span id="'+this.settings.pgeId+'_pge" class="'+this.settings.passLoginStyle+'"><OBJECT ID="' + this.settings.pgeId + '" CLASSID="CLSID:' + this.pgeditIEClassid + '" codebase="' 
					        +this.settings.pgePath+ this.pgeditIECab + '" onkeydown="if(13==event.keyCode || 27==event.keyCode)'+this.settings.pgeOnkeydown+';" class="' + this.settings.pgeClass + '">' 
					...
					var pgeOcx='<embed onmouseover="'+ff+'" onfocus="'+this.settings.pgeOnfocus+'" input_0 ="'+license+'" onblur="'+this.settings.pgeOnblur+'" ID="' + this.settings.pgeId + '"  maxlength="'+this.settings.pgeMaxlength+'" input_2="'+this.settings.pgeEreg1+'" input_3="'+this.settings.pgeEreg2+'" edittype="'+this.settings.pgeEdittype+'" type="application/spdbpass-guard-x" tabindex="'+this.settings.pgeTabindex+'" class="' + this.settings.pgeClass + '" ';
					...
					this.checkInstall(1,function(obj){
						...
						if((obj.getConvertVersion(objVersion)<obj.getConvertVersion(PGEdit_Edge_VERSION))&&PGEdit_Update=="1"){
							...
							var fos = "";
							if(obj.osBrowser == 10&&obj.settings.pgeEdittype==1){
								mingflag = true;
								$("#"+id+"_down").parent().html('<input type="text" onfocus="pgeCtrl.openProt(\''+winId+'\',this.id,true);pgeCtrl.setCX(this);'+obj.settings.pgeOnfocus+'" onkeydown="pgeCtrl.setSX(event,\''+obj.settings.pgeOnkeydown+'\',this);pgeCtrl.setCX(this);" onclick = "this.focus();pgeCtrl.setCX(this)" onblur = "pgeCtrl.closeProt(\''+winId+'\',this.id);'+obj.settings.pgeOnblur+'" id = "'+id+'" style="-webkit-ime-mode:disabled"  class="' + obj.settings.pgeClass + '"/>');
							
							}else if(obj.osBrowser == 10&&obj.settings.pgeEdittype!=1){
								mingflag = false;
								if(obj.osBrowser == 10){
									 fos = "this.type='password';";
								}
								$("#"+id+"_down").parent().html('<input type="text" onfocus="'+fos+'pgeCtrl.openProt(\''+winId+'\',this.id,false);pgeCtrl.setCX(this);'+obj.settings.pgeOnfocus+'" onkeydown="pgeCtrl.setSX(event,\''+obj.settings.pgeOnkeydown+'\',this);" onclick = "pgeCtrl.setCX(this)" onblur = "pgeCtrl.closeProt(\''+winId+'\',this.id);'+obj.settings.pgeOnblur+'" id = "'+id+'" style="ime-mode:disabled" class="' + obj.settings.pgeClass + '"/>');
								
							}else if(obj.osBrowser == 11&&obj.settings.pgeEdittype!=1){
								...
								$("#"+id+"_down").parent().html('<input type="text" onfocus="'+fos+'pgeCtrl.openProt(\''+winId+'\',this.id,true);pgeCtrl.setCX(this);'+obj.settings.pgeOnfocus+'" onkeydown="pgeCtrl.setSX(event,\''+obj.settings.pgeOnkeydown+'\',this);" onclick = "pgeCtrl.setCX(this)" onblur = "pgeCtrl.closeProt(\''+winId+'\',this.id);'+obj.settings.pgeOnblur+'" id = "'+id+'" style="ime-mode:disabled" class="' + obj.settings.pgeClass + '"/>');
							}
							...
							if(obj.osBrowser == 10 || obj.osBrowser == 11){
								...
								if(o != null){
									if(obj.settings.pgePlaceholder!=""){
										$(o).attr('placeholder',obj.settings.pgePlaceholder);
									}
									if(obj.osBrowser == 11&&obj.settings.pgeEdittype!=1){
					...
					/*if(window.top.isLogined == false && document.getElementById("loginpage")){
						return '<div><div id="'+this.settings.pgeId+'_down">'
	    						+'<a href="'+this.settings.pgePath+this.pgeditFFExe+'" target="_blank" style="text-decoration:underline;" class="ft-activex">'+this.pgeDownText+'</a>'
	    					+'</div>'
	    					+'<div class="clear"></div></div>';
			    	}else{
			    		return '<div><div id="'+this.settings.pgeId+'_down" class="'+this.settings.pgeInstallClass+'" style="text-decoration:underline;line-height:25px;"><a href="'+this.settings.pgePath+this.pgeditFFExe+'" target="_blank" style="text-decoration:underline;" class="ft-activex">请安装安全控件</a></div></div>';
			    	}*/
					return '<div style="display: inline-block;"><div id="'+this.settings.pgeId+'_down" class="'+this.settings.pgeInstallClass+'"><a class="ft-activex" style="text-decoration: underline;" href="'+this.settings.pgePath+this.pgeditFFExe+'">'+this.pgeDownText+'</a></div></div>';
				} else {
					return '<div><div id="'+this.settings.pgeId+'_down" class="'+this.settings.pgeInstallClass+'" style="text-decoration:underline;">暂不支持此浏览器</div></div>';
				}				
			},
			...
			getDownHtml : function() {
				if (this.osBrowser==1 || this.osBrowser==3) {
					return '<div id="'+this.settings.pgeId+'_down" class="'+this.settings.pgeInstallClass+'" ><a class="ft-activex" style="text-decoration: underline;" href="'+this.settings.pgePath+this.pgeditIEExe+'">'+this.pgeDownText+'</a></div>';
				} else if (this.osBrowser==2 || this.osBrowser==6 || this.osBrowser==8 || this.osBrowser == 10 || this.osBrowser == 11) {
					return '<div id="'+this.settings.pgeId+'_down" class="'+this.settings.pgeInstallClass+'" style="text-decoration:underline;line-height:25px;"><a href="'+this.settings.pgePath+this.pgeditFFExe+'">'+this.pgeDownText+'</a></div>';
				} else {
					return '<div id="'+this.settings.pgeId+'_down" class="'+this.settings.pgeInstallClass+'" style="text-decoration:underline;">暂不支持此浏览器</div>';
				}			
			},
			//清空密码
			pwdClear : function() {
			...
			initControl : function(id) {
				...
				INCJSON.data.oem_900=this.settings.pgeCert1;
				INCJSON.data.oem_901=this.settings.pgeCert2;
				INCJSON.data.oem_902=this.settings.setDictionaryFilter;
				INCJSON.data.oem_903=this.settings.setTimes;
			...
			//开启密码控件保护
			openProt : function(id,inputID,flag) {
				...
				if(typeof onceInterv[id] == "string"){//监听焦点切出
					...
					onceInterv[id] = window.setInterval("pgeCtrl.intervlOut(\""+id+"\",\""+inputID+"\"," + flag +")",800);	
					iterArray.push(onceInterv[id]);
				}
			},
			//心跳监测
			intervlOut : function(id,inputID,flag) {
					...
					success : function(x) {
						 ...
						 if(flag == true){
							 ...
							 if(len>code){
								 ...
								 va.value=va.value.substring(0, code);
							 }
