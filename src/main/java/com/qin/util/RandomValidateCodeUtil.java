...
//        使用reditTemplate设置过期时间
//        redisTemplate.opsForValue().set("RANDOMCODEKEY", randomString, 60L, TimeUnit.SECONDS);
//        redisTemplate.opsForValue().set("RANDOMCODEKEY", randomString, 1000L, TimeUnit.DAYS);
        map.put("RANDOMCODEKEY",randomString);
        g.dispose();
        try {
            // 将内存中的图片通过流动形式输出到客户端
